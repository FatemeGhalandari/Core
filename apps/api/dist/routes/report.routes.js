...
import { attachCurrentUser, requireRole, requireUser, } from "../middleware/auth.js";
...
async function getDemoOrganizationId() {
    ...
    if (!organization) {
        throw new Error("Demo organization not found. Run the database seed first.");
    }
...
reportRouter.get("/operations", ...requireReportViewer, async (_req, res, next) => {
    try {
        ...
        const [totalCases, openCases, closedCases, highPriorityCases, waitingCases, unassignedCases, overdueCases, customerCount, customersWithOpenCases, openCaseAges, workflowStatuses, caseCategories, sourceGroups, priorityGroups, owners, unassignedOpenCases, recentlyClosedCases,] = await Promise.all([
            prisma.case.count({
                where: {
                    organizationId,
                },
        ...
        const averageOpenAge = openCaseAges.length === 0
            ? 0
            : Math.round(openCaseAges.reduce((total, caseItem) => total + getAgeInDays(caseItem.createdAt), 0) / openCaseAges.length);
        const agingBuckets = [
            {
                label: "0-2 days",
                count: openCaseAges.filter((caseItem) => getAgeInDays(caseItem.createdAt) <= 2).length,
            },
            {
                label: "3-7 days",
            ...
            {
                label: "8+ days",
                count: openCaseAges.filter((caseItem) => getAgeInDays(caseItem.createdAt) >= 8).length,
            },
        ];
        const statusBreakdown = workflowStatuses.reduce((result, status) => {
            result[status.name] = status._count.cases;
            return result;
        }, {});
        const categoryBreakdown = caseCategories.reduce((result, category) => {
        ...
        const sourceBreakdown = sourceGroups.reduce((result, sourceGroup) => {
            result[sourceGroup.source] = sourceGroup._count._all;
            return result;
        }, {});
        const priorityBreakdown = priorityGroups.reduce((result, priorityGroup) => {
            result[priorityGroup.priority] = priorityGroup._count._all;
            return result;
        }, {});
        const ownerBreakdown = owners.reduce((result, owner) => {
